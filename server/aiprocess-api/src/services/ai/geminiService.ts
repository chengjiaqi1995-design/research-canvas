...
function normalizeMetadataParticipants(rawValue: unknown): 'management' | 'buyside' | 'expert' | 'sellside' | 'earnings' {
  const compact = String(rawValue || 'management')
    .trim()
    .toLowerCase()
    .replace(/[\s_\-./|,，、;；:：()[\]{}]+/g, '');
  ...
  if (compact.includes('buyside') || compact.includes('买方')) {
    return 'buyside';
  }
  if (compact.includes('sellside') || compact.includes('卖方') || compact.includes('券商')) {
    return 'sellside';
  }
  if (compact.includes('expert') || compact.includes('专家')) {
    return 'expert';
  }
  if (compact.includes('management') || compact.includes('mgmt') || compact.includes('管理层')) {
    return 'management';
  }
...
export async function extractMetadataWithGemini(
  ...
  try {
    ...
    // 笔记类型：AI 应该返回 "management"、"buyside"、"expert"、"sellside" 或 "earnings" 之一；兼容旧值和模型变体。
