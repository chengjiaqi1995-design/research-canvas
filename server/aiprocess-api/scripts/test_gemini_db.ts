...
async function main() {
  ...
  const orgs = await prisma.transcription.findMany({
    where: { organization: { in: ['BP', 'Chevron', 'Baker Hughes', 'Baker'] } },
  ...
  const unmatchedOrgs = Object.keys(orgContexts);
  const batch = unmatchedOrgs.map(o => `公司简称: "${o}" | 摘要上下文: "${orgContexts[o].replace(/\n/g, ' ')}"`);
...
只返回JSON，绝对不要带有 \`\`\`json 等任何格式标记，绝对不要有其他任何说明文字。`;
  ...
  console.log("Sending prompt to Gemini with actual DB context...");
