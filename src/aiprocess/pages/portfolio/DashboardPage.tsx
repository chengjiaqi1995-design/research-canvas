...
import type { PortfolioSummary, PositionWithRelations, SummaryByDimension } from "../../lib/portfolio-types";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/portfolio/earnings`, { headers: getHeaders() }).then(r => r.json()).then(raw => {
  ...
  async function refreshPrices() {
    ...
    try {
      await fetch(`${API_BASE}/portfolio/prices/update`, { method: "POST", headers: getHeaders() });
      refreshData();
    } finally {
  ...
  function refreshData() {
    Promise.all([
      ...
      setPositions(Array.isArray(pos?.data) ? pos.data : Array.isArray(pos) ? pos : []);
    });
  }
  ...
  async function saveAum() {
    ...
    await fetch(`${API_BASE}/portfolio/settings`, {
      method: "PUT",
      headers: getHeaders(),
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-xl font-normal tracking-tight">Dashboard</h1>
          ...
          <Card key={card.label} className="py-3 card-accent-top">
            <CardContent className="px-4 py-0">
              ...
              {card.label === "AUM" && editingAum ? (
                ...
                <p className={`font-serif text-lg font-semibold ${card.color ?? ""} ${card.label === "AUM" ? "cursor-pointer hover:text-[var(--accent)] group inline-flex items-center gap-1 transition-colors" : ""}`}
