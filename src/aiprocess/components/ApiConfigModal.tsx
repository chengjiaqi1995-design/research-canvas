...
1. 内容去向判断：
   - 某个已知公司（有专属 scope）的具体信息 → 只放到该公司的 scope
   - 行业级宏观信息（趋势、政策、技术路线等不涉及特定公司的分析）→ 行业 scope
   - 多公司横向对比 → 行业 scope
   - 没有专属 scope 的公司相关信息 → 融入行业 scope 的文章中提及即可

2. ⚠️ 绝对不能重复：如果某公司有专属 scope，该公司的具体数据只写入公司 scope，绝不在行业 scope 中重复。

3. ⚠️ 行业 scope 不为单个公司建立专属页面。

4. ⚠️ 页面类型限制：文章标题的类型标签（如 [xxx]）必须严格使用上方 PAGE TYPES 中列出的类型，绝对不能使用未列出的标签。
