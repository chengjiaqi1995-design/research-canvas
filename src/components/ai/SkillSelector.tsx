    ...
    <div className="relative w-full">
      <button
        ...
        className={`flex items-center justify-between w-full px-2.5 py-1.5 text-xs text-left border rounded transition-colors ${
          ...
          selectedSkillId ? 'bg-violet-50 border-violet-200 text-violet-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'
        }`}
      >
        <span className="flex items-center gap-1.5 truncate">
          <FileCode2 size={12} className={selectedSkillId ? "text-violet-500" : "text-slate-400"} />
          ...
          <div className="absolute left-0 right-0 top-8 z-50 bg-white rounded-lg shadow-xl border border-slate-200 py-1 max-h-[250px] flex flex-col">
            ...
            <div className="px-2 py-1.5 border-b border-slate-100 shrink-0">
              <button
                ...
                className="flex items-center justify-center gap-1.5 w-full px-2 py-1.5 text-xs text-violet-600 bg-violet-50 hover:bg-violet-100 rounded transition-colors font-medium border border-violet-100"
              >
                <Upload size={12} />
                上传新方法论 (.md/.txt)
              </button>
              ...
              {skills.map((skill) => (
                <div
                  ...
                  className={`flex items-center justify-between group w-full px-2 py-1.5 text-xs rounded transition-colors cursor-pointer ${
                    selectedSkillId === skill.id ? 'bg-violet-50 text-violet-700 font-medium' : 'text-slate-700 hover:bg-slate-50'
                  }`}
                  onClick={() => {
                    onSelect(skill.id);
                    setOpen(false);
                ...
                >
                  <div className="flex items-center gap-2 overflow-hidden">
                    <div className="w-3 shrink-0 flex items-center justify-center">
                      {selectedSkillId === skill.id && <CheckCircle2 size={12} className="text-violet-500" />}
                    </div>
