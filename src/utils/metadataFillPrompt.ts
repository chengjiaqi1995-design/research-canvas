...
- participants: 笔记类型，只能是 management / buyside / expert / sellside / earnings 之一；earnings 用于公司业绩点评/公司研究笔记
...
function migrateMetadataFillPrompt(prompt: string): string {
  return prompt
    .replace(
      /participants: 笔记类型，只能是 management \/ expert \/ sellside \/ company 之一；company 仅用于 AI 生成的公司点评\/公司研究笔记/g,
      'participants: 笔记类型，只能是 management / buyside / expert / sellside / earnings 之一；earnings 用于公司业绩点评/公司研究笔记'
    )
    .replace(
      /participants: 笔记类型，只能是 management \/ expert \/ sellside \/ earnings 之一；earnings 用于公司业绩点评\/公司研究笔记/g,
      'participants: 笔记类型，只能是 management / buyside / expert / sellside / earnings 之一；earnings 用于公司业绩点评/公司研究笔记'
    )
    .replace(/非上市公司格式：\[Private\] 公司名称，如 \[Private\] SpaceX/g, '非上市公司不要生成公司页名称，直接输出空字符串，不要输出 [Private]')
    .replace(/- 非上市:\s*\[Private\] 公司名称/g, '- 非上市: 不用写')
    .replace(/"participants"\s*:\s*"company"/g, '"participants":"earnings"')
    .replace(/participants["']?\s*[:：]\s*["']?company["']?/gi, 'participants:"earnings"');
}
...
for (const companies of Object.values(INDUSTRY_COMPANIES)) {
  for (const c of companies) {
    if (c.startsWith('[') && !/^\s*\[private\]/i.test(c) && SAMPLE_COMPANIES.length < 20) {
...
export function getMetadataFillPrompt(): string {
  try {
    ...
    if (saved && saved.trim()) {
      const migrated = migrateMetadataFillPrompt(saved);
      if (migrated !== saved) {
        localStorage.setItem('metadataFillPrompt', migrated);
      }
      return migrated;
    }
  } catch {}
...
export async function saveMetadataFillPrompt(prompt: string) {
  const migrated = migrateMetadataFillPrompt(prompt);
  localStorage.setItem('metadataFillPrompt', migrated);
  try {
    await aiApi.saveSettings({ metadataFillPrompt: migrated });
  } catch (e) {
...
export async function syncMetadataFillPrompt(): Promise<void> {
  try {
    ...
    if (res?.metadataFillPrompt) {
      const migrated = migrateMetadataFillPrompt(res.metadataFillPrompt);
      localStorage.setItem('metadataFillPrompt', migrated);
      if (migrated !== res.metadataFillPrompt) {
        await aiApi.saveSettings({ metadataFillPrompt: migrated });
      }
    } else {
      const local = localStorage.getItem('metadataFillPrompt');
      if (local && local !== DEFAULT_METADATA_FILL_PROMPT) {
        await aiApi.saveSettings({ metadataFillPrompt: migrateMetadataFillPrompt(local) });
      }
    }
  } catch {}
}
...
- 非上市公司不要生成公司页名称，直接输出空字符串，不要输出 [Private]
- 中国公司用中文全称，外国公司用英文全称
- 不要加引号
